...
export function DashboardClient({ initialPlans, user }: DashboardClientProps) {
    ...
    const stats = useMemo(() => {
        // Guard against undefined fields to avoid NaN in UI
        const totalSkills = plans.reduce((acc, plan) => acc + (plan.skillCount ?? 0), 0);
        const completedSkills = plans.reduce(
            (acc, plan) => acc + Math.floor((plan.skillCount ?? 0) * ((plan.progress ?? 0) / 100)),
            0
        );
        const avgProgress = plans.length > 0
            ? Math.round(
                  plans.reduce((acc, plan) => acc + (plan.progress ?? 0), 0) / plans.length
              )
            : 0;
            ...
            {filteredPlans.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <motion.button
                        ...
                        onClick={() => setIsCreateOpen(true)}
                        className="glass-card border-2 border-dashed border-gray-300 dark:border-gray-600 hover:border-primary-500 dark:hover:border-primary-500 p-6 flex flex-col items-center justify-center gap-3 min-h-[200px] transition-colors group"
                    >
                ...
                >
                    <div className="w-24 h-24 mx-auto mb-6 rounded-full bg-gray-900 dark:bg-gray-800 flex items-center justify-center">
                        <Plus className="w-10 h-10 text-white" />
                    </div>
                    <h2 className="text-2xl font-semibold mb-2 text-gray-900 dark:text-white">No career paths yet</h2>
